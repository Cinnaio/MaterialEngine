...
public final class SeedPouch implements Listener {
    ...
    private final NamespacedKey legacyIdentityKey;
    private final Set<Inventory> menus = ConcurrentHashMap.newKeySet();

    public SeedPouch(JavaPlugin plugin, CraftEngineHook hook, TeaStoryLang lang, HarvestMenuItems menuItems,
        ...
        legacyContentsKey = new NamespacedKey("materiaengine", "seed_pouch_contents");
        legacyIdentityKey = new NamespacedKey("materiaengine", "seed_pouch_identity");
    }
    ...
    Bag read(Player player, int slot) {
        ...
        try {
            ...
            if (bytes == null) bytes = pdc.get(legacyContentsKey, PersistentDataType.BYTE_ARRAY);
            ...
            String identity = pdc.get(identityKey, PersistentDataType.STRING);
            if (identity == null) identity = pdc.get(legacyIdentityKey, PersistentDataType.STRING);
            return new Bag(slot, snapshot, identity, contents);
