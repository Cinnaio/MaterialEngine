...
import com.github.cinnaio.teastory.config.HarvestToolsConfig.Crop;
import com.github.cinnaio.teastory.config.HarvestToolsConfig.Mode;
import com.github.cinnaio.teastory.config.HarvestToolsConfig.Tool;
import com.github.cinnaio.teastory.config.HarvestToolsConfig.Effect;
...
public final class HarvestToolsFeature implements Listener {
    ...
    public HarvestToolsFeature(JavaPlugin plugin, CraftEngineHook hook, BeaconEngineBridge beacon) {
        this(plugin, hook, new FruitRegrowth(plugin, hook), HarvestToolsConfig.load(plugin.getConfig()),
                ThreadLocalRandom.current(), beacon, null, null, null);
        regrowth.start();
    }

    public HarvestToolsFeature(JavaPlugin plugin, CraftEngineHook hook, BeaconEngineBridge beacon,
                               TeaStoryLang lang, HarvestStats stats, DefinitionFiles definitions) {
        this(plugin, hook, new FruitRegrowth(plugin, hook),
                HarvestToolsConfig.load(plugin.getConfig(), definitions.loadTools()),
                ThreadLocalRandom.current(), beacon, lang, stats, definitions);
        regrowth.start();
    }

    public HarvestToolsFeature(JavaPlugin plugin, CraftEngineHook hook, BeaconEngineBridge beacon,
                               TeaStoryLang lang, HarvestStats stats) {
        this(plugin, hook, new FruitRegrowth(plugin, hook), HarvestToolsConfig.load(plugin.getConfig()),
                ThreadLocalRandom.current(), beacon, lang, stats, null);
        regrowth.start();
    }
    ...
    HarvestToolsFeature(JavaPlugin plugin, CraftEngineHook hook, FruitRegrowth regrowth,
        ...
        this(plugin, hook, regrowth, config, random, beacon, null, null, null);
    }

    HarvestToolsFeature(JavaPlugin plugin, CraftEngineHook hook, FruitRegrowth regrowth,
                        HarvestToolsConfig config, RandomGenerator random, BeaconEngineBridge beacon,
                        TeaStoryLang lang, HarvestStats stats) {
        this(plugin, hook, regrowth, config, random, beacon, lang, stats, null);
    }
    ...
    public boolean reload() {
        plugin.reloadConfig();
        try {
            HarvestToolsConfig candidate = definitions == null
                    ? HarvestToolsConfig.load(plugin.getConfig())
                    : HarvestToolsConfig.load(plugin.getConfig(), definitions.loadTools());
