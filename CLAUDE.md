...
TeaStory 是 Paper/Folia 26.1.x 插件（已用 26.1.2 验证），用于承载 TeaStory 机器逻辑，并与 CraftEngine 内容包配合使用。Java 包名和主类已统一为 `com.github.cinnaio.teastory` / `TeaStoryPlugin`，旧名称只用于迁移和兼容。
